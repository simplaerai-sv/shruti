...
- 💻 **Розшифровка на вашому компʼютері** — без ключа, без акаунта, без мережі
  й без витрат. Працює на локальній моделі Whisper, яку застосунок завантажує
  один раз (**1,5 ГБ**, тож одразу після встановлення вона ще не готова).
  Година зустрічі розшифровується приблизно за три хвилини. Оскільки два
  канали розпізнаються окремо, хто що казав — **відомо, а не вгадано**; щоправда,
  у груповому дзвінку всі на тому боці — один співрозмовник.
...
## Перший запуск: оберіть, чим розшифровувати

**Записувати можна одразу після встановлення.** А щоб перетворити запис на
текст, потрібне одне з двох: хмарний сервіс або модель на вашому компʼютері.

| Спосіб | Вартість | Що варто знати |
|---|---|---|
| **На цьому компʼютері** | нічого й ніколи | Застосунок **один раз завантажує модель на 1,5 ГБ**. Поки це завантаження не завершиться, локальної розшифровки немає — самого лише встановлення застосунку недостатньо. Далі працює **без мережі й без акаунта**. |
...
| **[OpenAI](https://platform.openai.com/api-keys)** | платний, потрібна картка | Єдиний спосіб отримати **живу** транскрипцію та конспекти |

Локальна модель лягає в `data/models` поруч із застосунком, тож переносна
копія забирає її з собою. Завантаження перевіряється за розміром і
контрольною сумою: обірване ніколи не вдаватиме готове.

Ключ, якщо ви ним користуєтесь, вставляється в **Налаштування** (шестерня
вгорі праворуч) і зберігається лише на вашому компʼютері.

З чого почати: якщо можете зачекати на 1,5 ГБ завантаження — беріть **На цьому
компʼютері**, тоді нічого не покидає вашу машину і ніколи нічого не коштує.
Якщо транскрипт потрібен за пʼять хвилин — Google AI Studio.
...
- У режимі **«На цьому компʼютері» аудіо не покидає вашу машину взагалі** —
  розпізнавання відбувається локально, мережа при цьому не потрібна.
- Якщо ви обрали хмарний сервіс, аудіо надсилається **тільки** туди, чий ключ
  ви ввели (OpenAI або Google), — виключно для розпізнавання мовлення.
